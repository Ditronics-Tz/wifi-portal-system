                ...
                <div class="form-group">
                    ...
                    <select name="plan" id="plan" class="form-select" required>
                        ...
                        <?php foreach ($packages as $pkg): if (!$pkg['is_active']) continue; ?>
                            <option value="<?php echo (int) $pkg['id']; ?>" <?php echo ((int) $planId === (int) $pkg['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($pkg['name']); ?> — <?php echo number_format($pkg['price']); ?> TZS (<?php echo fmtDuration((int)$pkg['duration_seconds']); ?>, <?php echo !empty($pkg['data_quota_mb']) ? number_format($pkg['data_quota_mb']) . ' MB' : 'no quota — set one first'; ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
